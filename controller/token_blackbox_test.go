...
func (rest *TestTokenREST) TestInitTenantOK() {
	service, controller := rest.SecuredController()

	checkInitTenant(rest.T(), func(tenantContext *tenantContext) {
		controller.InitTenant = newInitTenant(tenantContext)
		rest.checkAuthorizationCode(service, controller, controller.Configuration.GetPublicOauthClientID(), "SOME_OAUTH2.0_CODE")
	})
}

func checkInitTenant(t *testing.T, callController func(tenantContext *tenantContext)) {
	tenantContext := &tenantContext{wg: &sync.WaitGroup{}}
	tenantContext.wg.Add(1)

	callController(tenantContext)

	if waitTimeout(tenantContext.wg, time.Second) {
		require.Fail(t, "Init tenant timed out")
	}
	require.True(t, tenantContext.done)
}
...
func newInitTenant(tenantContext *tenantContext) func(ctx context.Context) error {
	return func(ctx context.Context) error {
		defer tenantContext.wg.Done()
		tenantContext.done = true
		return nil
	}
}
